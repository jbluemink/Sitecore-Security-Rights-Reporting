...
namespace Security.Rights.Reporting.sitecore_modules.Shell.Security_Rights_Reporting
{
    public static class RolesExport
    {
        ...
        private static void Step1(Literal rolesexport)
        {
            ...
            rolesexport.Text += "<p>With this export roles tool you can export one or more roles including all item rights that are set. This tool can help to move your rights through the DTAP-street. At the import rules are created if they not exist already. Items are matched by the item path, not the id. Only the security field is modified, and only the rights for the selected roles, rights are only added, rights not in the export will not modify or delete. Note only items in the master database. Note, if you only want to export the roles without rights, you can use a normal Sitecore Package. For exporting users see <a href=\"https://kb.sitecore.net/articles/242631\" target=\"_blank\">Transferring user passwords between Sitecore instances</a></p>";
